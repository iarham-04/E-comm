...
const STATUS_COLORS: Record<string, string> = {
  PLACED: 'bg-neutral-100 text-neutral-700 border border-neutral-300',
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-neutral-200 pb-5 gap-4">
        <div>
          <span className="text-[10px] font-bold text-neutral-700 uppercase tracking-widest bg-neutral-100 px-2 py-0.5 rounded-full border border-neutral-300 inline-block mb-1">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            disabled={loading}
            className="p-2.5 bg-white hover:bg-[#fafafa] text-neutral-700 rounded-xl border border-neutral-200 shadow-xs transition"
          ...
          <Link
            ...
            className="bg-gradient-to-r from-neutral-900 to-neutral-800 hover:from-neutral-800 hover:to-neutral-700 text-white text-xs font-black px-4 py-2.5 rounded-xl shadow-md shadow-neutral-300/20 transition-all"
          >
            + Add Product
          </Link>
          <Link
            href="/admin/orders"
            className="bg-white hover:bg-[#fafafa] text-neutral-800 text-xs font-bold px-4 py-2.5 rounded-xl border border-neutral-200 shadow-xs transition"
          >
            Manage Orders
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        {[
          ...
          return (
            <div
              key={m.label}
              className="bg-white rounded-2xl border border-neutral-200/90 shadow-xs p-4 space-y-2 hover:border-neutral-300 transition"
            >
              ...
              <p className="text-xl sm:text-2xl font-black text-neutral-900">{m.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Revenue Sparkline */}
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-neutral-200/90 shadow-xs p-5 sm:p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-bold text-neutral-900">Gross Revenue Trend</h2>
              <p className="text-xs text-neutral-500 mt-0.5">Last 7 days performance (₹ INR)</p>
            </div>
            ...
            {REVENUE_BARS.map((val, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center space-y-1.5 h-full justify-end">
                  ...
                  <span className="text-[10px] text-neutral-500 font-semibold">{days[i]}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-neutral-200/90 shadow-xs p-5 sm:p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-neutral-900">Top Performing Artifacts</h2>
            <Link href="/admin/products" className="text-xs font-bold text-neutral-600 hover:underline">
              Full Catalog →
            </Link>
          </div>
          <div className="space-y-2.5">
            {[
              ...
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded-xl bg-[#fafafa] border border-neutral-100 hover:bg-neutral-100/80 transition"
              >
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  <span className="text-xs font-black text-neutral-700 w-5 flex-shrink-0">
                    #{i + 1}
                  </span>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-neutral-900 line-clamp-1">{p.name}</p>
                    <p className="text-[10px] text-neutral-500">{p.orders} completed orders</p>
                  </div>
                </div>
                <span className="text-xs font-black text-neutral-900 ml-4 flex-shrink-0">
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-neutral-200/90 shadow-xs p-5 sm:p-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="text-xs font-bold text-neutral-700 bg-neutral-50 border border-neutral-200 px-3 py-1 rounded-full self-start sm:self-auto">
          ...
          {FUNNEL_EVENTS.map((event, idx) => {
            ...
            return (
              <div key={event.label} className="flex items-center gap-3 group">
                ...
                <div className="flex-1 relative h-5 sm:h-6 bg-neutral-100 rounded-full overflow-hidden border border-neutral-200">
                  <div
      ...
      {/* Recent Orders */}
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-neutral-200/90 p-5 sm:p-6 shadow-xs space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-sm font-bold text-neutral-900">Recent Customer Orders</h2>
          <Link href="/admin/orders" className="text-xs font-bold text-neutral-600 hover:underline">
            View All Orders →
          </Link>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-[#fafafa] border-b border-neutral-200 text-neutral-600 uppercase font-bold tracking-wider text-[10px]">
              <tr>
            ...
            <tbody className="divide-y divide-neutral-100">
              {(recentOrders.length > 0 ? recentOrders : [
                ...
                <tr key={o.id} className="hover:bg-[#fafafa] transition-colors">
                  <td className="p-3 font-mono font-bold text-neutral-700">#{o.id}</td>
                  <td className="p-3 font-semibold text-neutral-900">
                    {o.user?.name || o.guestEmail || o.customer || 'Guest Collector'}
                  </td>
                  <td className="p-3 font-bold text-neutral-900">₹{Number(o.total).toLocaleString('en-IN')}</td>
                  <td className="p-3">
                    <span
