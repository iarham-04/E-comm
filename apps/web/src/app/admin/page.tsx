...
const STATUS_COLORS: Record<string, string> = {
  PLACED: 'bg-amber-500/10 text-amber-300 border border-amber-500/30',
  CONFIRMED: 'bg-blue-500/10 text-blue-300 border border-blue-500/30',
  SHIPPED: 'bg-purple-500/10 text-purple-300 border border-purple-500/30',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-5 gap-4">
        <div>
          <span className="text-[10px] font-bold text-amber-400 uppercase tracking-widest bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20 inline-block mb-1">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs font-black px-4 py-2.5 rounded-xl shadow-lg shadow-amber-500/20 transition-all"
          >
            + Add Product
          </Link>
          <Link
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          },
          {
            ...
            icon: AlertTriangle,
            accent: metrics.lowStockCount > 0 ? 'text-rose-400' : 'text-emerald-400',
            bg: metrics.lowStockCount > 0 ? 'bg-rose-500/10 border-rose-500/20' : 'bg-emerald-500/10 border-emerald-500/20',
          },
        ].map((m) => {
          const Icon = m.icon;
          return (
            <div
              key={m.label}
              className="bg-slate-950 rounded-2xl border border-slate-800 shadow-xl p-4 space-y-2 hover:border-slate-700 transition"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Revenue Sparkline */}
        <div className="bg-slate-950 rounded-3xl border border-slate-800 shadow-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-bold text-white">Gross Revenue Trend</h2>
              <p className="text-xs text-slate-400 mt-0.5">Last 7 days performance (₹ INR)</p>
            </div>
            ...
            {REVENUE_BARS.map((val, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center space-y-1.5 h-full justify-end">
                  <div
                    className="w-full rounded-t-lg bg-slate-800 hover:bg-amber-400 transition-all duration-300 relative group cursor-pointer"
                    style={{ height: `${pct}%` }}
                  >
                    <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-900 border border-slate-700 text-white text-[9px] px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap pointer-events-none z-10">
        ...
        <div className="bg-slate-950 rounded-3xl border border-slate-800 shadow-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-white">Top Performing Artifacts</h2>
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-850"
              >
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  <span className="text-xs font-black text-amber-400 w-5 flex-shrink-0">
                    #{i + 1}
                  </span>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-white line-clamp-1">{p.name}</p>
                    <p className="text-[10px] text-slate-400">{p.orders} completed orders</p>
                  </div>
                </div>
                <span className="text-xs font-black text-white ml-4 flex-shrink-0">
      ...
      <div className="bg-slate-950 rounded-3xl border border-slate-800 shadow-xl p-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="text-xs font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full self-start sm:self-auto">
          ...
          {FUNNEL_EVENTS.map((event, idx) => {
            ...
            return (
              <div key={event.label} className="flex items-center gap-3 group">
                ...
                <div className="flex-1 relative h-6 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
                  <div
            ...
            <tbody className="divide-y divide-slate-850">
              {(recentOrders.length > 0 ? recentOrders : [
                ...
                <tr key={o.id} className="hover:bg-slate-900/60 transition-colors">
                  ...
                  <td className="p-3 font-bold text-white">₹{Number(o.total).toLocaleString('en-IN')}</td>
                  <td className="p-3">
                    <span
                      className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                        STATUS_COLORS[o.status] || 'bg-slate-800 text-slate-400'
                      }`}
                    >
                      {o.status}
                    </span>
