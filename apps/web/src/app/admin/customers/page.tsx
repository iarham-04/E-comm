    ...
    <div className="space-y-6 text-xs text-slate-100">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-200 pb-4">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold text-neutral-900 tracking-tight mt-1 flex items-center gap-2">
            <span>Customer Management &amp; Staff Notes</span>
            <span className="text-[10px] bg-neutral-900/20 text-neutral-600 border border-amber-500/40 px-2 py-0.5 rounded-full font-bold uppercase flex items-center gap-1">
              <Lock className="w-3 h-3" /> OWNER ONLY
            </span>
          </h1>
          <p className="text-xs text-neutral-400">Maintain confidential internal staff notes (VIP status, preferences, support history). Never exposed to customers.</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {customers.map((c) => (
          <div key={c.id} className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-900 pb-3">
              <div>
                <h3 className="font-bold text-neutral-900 text-sm">{c.name || 'Anonymous Customer'}</h3>
                <p className="text-xs font-mono text-neutral-400">{c.email} · Registered {new Date(c.createdAt).toLocaleDateString()}</p>
              </div>
            ...
            <div>
              <label className="block text-[11px] font-bold text-neutral-400 mb-1 flex items-center gap-1">
                ...
                <span className="text-[9px] text-neutral-600/80 font-mono">(CONFIDENTIAL — Staff Only)</span>
              </label>
              <textarea
                rows={2}
                value={notesMap[c.id] ?? ''}
                ...
                className="w-full px-3 py-2 bg-[#fafafa] border border-neutral-200 rounded-xl text-neutral-900 text-xs focus:outline-none focus:border-amber-400"
              />
            </div>
          </div>
        ))}
