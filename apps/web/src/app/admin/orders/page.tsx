...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  PLACED:    { label: 'Placed',    color: 'bg-neutral-100 text-neutral-700',   icon: Package },
  CONFIRMED: { label: 'Confirmed', color: 'bg-blue-100 text-blue-800',     icon: CheckCircle2 },
  SHIPPED:   { label: 'Shipped',   color: 'bg-purple-100 text-purple-800', icon: Truck },
  DELIVERED: { label: 'Delivered', color: 'bg-emerald-100 text-emerald-700', icon: Star },
    ...
    <div className="space-y-6 text-xs text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 text-xs bg-white border border-neutral-200 rounded-xl text-neutral-900 focus:outline-none focus:border-amber-400"
          />
        </div>
      </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-900">
            {filtered.map((o) => (
              <tr key={o.id} className="hover:bg-[#fafafa]/50 transition-colors">
                <td className="py-3 px-4 font-mono font-bold text-neutral-900">#{o.id.slice(-6).toUpperCase()}</td>
                <td className="py-3 px-4">
                  <p className="font-bold text-neutral-900">{o.user?.name || o.guestEmail || 'Collector'}</p>
                  <p className="text-[10px] text-neutral-500 font-mono">{o.user?.email || o.guestEmail || '—'}</p>
                </td>
                ...
                <td className="py-3 px-4">
                  <select
                    ...
                    className="px-2 py-1 bg-[#fafafa] border border-neutral-200 rounded-lg text-neutral-900 font-bold text-[11px]"
                  >
                    {STATUS_FLOW.map((s) => (
                ...
                <td className="py-3 px-4 text-[11px]">
                  {o.trackingNumber ? (
                    ...
                    <button onClick={() => { setTrackingModalOrder(o); setTrackingNumber(o.trackingNumber || ''); }} className="text-neutral-600 font-bold hover:underline">
                      + Add Tracking
                    </button>
                  )}
                </td>
                <td className="py-3 px-4 text-right space-x-2">
                  ...
                  {o.paymentStatus !== 'REFUNDED' && (
                    <button
                      ...
                      className="p-1.5 bg-rose-950 hover:bg-rose-900 text-rose-600 rounded-lg"
                      title="Issue Refund"
                    >
      ...
      {refundModalOrder && (
        <div className="fixed inset-0 bg-white/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fafafa] border border-neutral-200 rounded-2xl max-w-md w-full p-6 space-y-4 text-slate-100">
            <h2 className="font-bold text-base text-neutral-900">Issue Order Refund (Order #{refundModalOrder.id.slice(-6).toUpperCase()})</h2>
            <form onSubmit={handleRefundSubmit} className="space-y-3">
              <div>
                <label className="block font-bold text-neutral-400 mb-1">Refund Amount (₹) *</label>
                <input
                  required
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="block font-bold text-neutral-400 mb-1">Refund Reason *</label>
                <textarea
                  required
                  rows={2}
      ...
      {trackingModalOrder && (
        <div className="fixed inset-0 bg-white/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fafafa] border border-neutral-200 rounded-2xl max-w-md w-full p-6 space-y-4 text-slate-100">
            <h2 className="font-bold text-base text-neutral-900">Add Shipping Tracking Info</h2>
            <form onSubmit={handleTrackingSubmit} className="space-y-3">
              <div>
                <label className="block font-bold text-neutral-400 mb-1">Carrier Name *</label>
                <input
                  required
                  type="text"
                  value={carrier}
              ...
              <div>
                <label className="block font-bold text-neutral-400 mb-1">Tracking / Waybill Number *</label>
                <input
                  required
                  type="text"
